...
b_i &\sim& N(0, D)\\
\epsilon_{i} &\sim& N(0, \sigma I_{k_i}) \mbox{~where} I_{k_i} = (1,...,1), k_i \mbox{times}\\
...
&=& \prod_{i=1}^{N} \frac{1}{(2\pi \sigma)^{k/2}}\exp\left\{-\frac{(y_i -X_{ij}\beta - Z_{ij}b_i)(y_i -X_{ij}\beta - Z_{ij}b_i)^T}{2\sigma^{2}}\right\}\\
&\times& \frac{1}{(2\pi)^{k/2} \det(D)^{1/2}}\exp\left\{\frac{- b_i D^{-1} b_i^T}{2}\right\}
\end{eqnarray*}
...
&=& \prod_{i=1}^{N} \frac{1}{(2\pi)^{k/2}det(\sigma^{2}ID)^{1/2}}\exp\left\{-\frac{(y_i -X_{ij}\beta - Zb)(y_i -X_{ij}\beta - Z_{ij}b_i)^T}{2\sigma^{2}}\right\} \\
&\times& \frac{1}{(2\pi)^{k/2} \det(D)^{1/2}}\exp\left\{-\frac{b_i D^{-1} b_i^T}{2}\right\}\\
&=& \prod_{i=1}^{N} \frac{1}{(2\pi)^{k/2}det(\sigma^2 Id)^{1/2}}\frac{1}{(2\pi)^{k/2} \det(D)^{1/2}} \exp\left\{-\frac{(y_i -X_{ij}\beta - Z_{ij}b_i)(y_i -X_{ij}\beta - Z_{ij}b_i)^T + \sigma^2 b_iD^{-1}b_i^T}{2\sigma^{2}}\right\}
\end{eqnarray*}
...
\begin{eqnarray*}
f_{miss}(b|y;\beta,D,\sigma) &\propto& f(y|b) f(b|D) \\
&\propto& \prod_{i=1}^{N} \frac{1}{\sigma^{k/2}}\frac{1}{\det(D)^{1/2}} \exp\left\{-\frac{[b_i - \Omega_i\sigma^{-2} Z_i^T(y_i - X_{ij}\beta)]^T \Omega_i^{-1}[b_i - \Omega_i\sigma^{-2} Z_i^T(y_i - X_{ij}\beta)]}{2}\right\}
\end{eqnarray*}
...
&=& y_i^Ty_i - y_i^TX_i\beta - y_i^TZ_ib_i - (X_i\beta)^Ty_i + (X_i\beta)^T(X_i\beta) + (X_i\beta)^TZ_ib_i - y_i^T(Z_ib_i) + (X_i\beta)Z_ib_i + Trace(Z_i^TZ_ib_ib_i^T))\\
\end{eqnarray*}
...
\begin{eqnarray*}
Q(\theta, \theta^{(s)}) &=& \sum_{i=1}^N \left(-\frac{1}{2}\log(det(\sigma Id)) -\frac{1}{2}\log(\det(D)) + (X_i\beta)^TZ_iE(b_i|y;\theta^{(s)}) - y_i^T Z_iE(b_i|y;\theta^{(s)}) + \right)\\
&+& \sum_{i=1}^N \left(\frac{1}{2}Trace(Z_i^TZ_iE(b_ib_i^T|y;\theta^{(s)})) + \frac{(y_i - X_i\beta)^T(y_i - X_i\beta)}{2\sigma^{2}}\right)
\end{eqnarray*}
where
\begin{eqnarray*}
E(b_i|y;\theta^{(s)}) &=&  \Omega_i Z_i^T(y_i - X_{ij}\beta^{(s)}) =: \mu_i^{(s)}\\
E(b_ib_i^T|y;\theta^{(s)}) &=& Cov(b_i,b_i^T) + E(b_i|y;\theta^{(s)}) E(b_i|y;\theta^{(s)})^T = \Omega_i^{(s)} + \mu^{(s)}(\mu^{(s)})^T=: \mu_{ii}
\end{eqnarray*}
...
\[
\sum_{i=1}^{N}(X_i)^T(y_i - X_i\beta - Z_iE(b_i|y;\theta^{(s)}) + (y_i - X_i\beta - Z_iE(b_i|y;\theta^{(s)})) = 0
\]
the solution is
\[
